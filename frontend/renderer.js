...
function showTab(evt, tabName) {
    ...
    // if user is on album ratings tab then set the songs path and send the request to main.js for the song's album
    if (tabName === 'albumRatings') {
        ...
        //display the returned values from main
        displayRatedAlbums();
    }
    } else {
...
// when the app is loaded up set / find these values / elements
document.addEventListener('DOMContentLoaded', () => {
    ...
    // refresh each tab every 5 seconds
    setInterval(() => {
        updateAllContainers();
...
// function that is called inside the 5-sec loop to update each html tab
...
// send the songs request to main.js
...
// updates the play / pause button display
function updatePlayPauseButton() {
    const playBtn = document.querySelector('.play-button');
    playBtn.innerHTML = isPlaying ? '&#10074;&#10074;' : '&#9658;'; // Play or pause symbols
}

// gets next song in index
function nextSong() {
    if (isPlaylistActive) {
        nextSongPlaylist();
    } else {
...
// updates the volume from the html element
function updateVolume(value) {
    audioPlayer.volume = value;
}

// loads songs by index 
...
// updates the "now playing" html element
...
// updates the "now playing" html element for instrumentals or vocals or clean songs
...
//the instrumental or vocal or clean song's name based off path
...
/*
    ...
    (this)
*/
// sends the request to main.js for python logic to be executed
...
// creates html elements for each song and each instrumental that already exists
...
// gets the resulting data from main.js 
...
// creates the buttons for each song and when clicked sends the request to main.js
...
// VOCALS
// sends request to main.js to get the vocals from the python file
...
// creates a button for each song and sends the request to main when clicked on the button
...
// the response from main.js for vocal songs
...
// PLAYLISTS
// gets all songs for the playlist
...
// displays all songs that are to be added to the playlist
...
// when listening to a playlist only the playlist songs are played
...
// finds the user-made playlist by id
function getPlaylistById(playlistId) {
  return playlists.find(p => p.id === playlistId);
}

// adds a song to the playlist id
function addSongToPlaylist(playlistId, song) {
...
// sets the rating value for the song
...
// displays the rated songs
// creates html elements that display the rated song
...
// creates html elements to display each album from main.js meta-data
...
// sends the request for the album data to main.js
...
// sets the rating value to the album
